...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                UPDATE forum_posts
                SET title = ?, content = ?, excerpt = ?, author = ?, category_id = ?, icon = ?, status = ?, replies = ?, views = ?, updated_at = NOW()
                WHERE id = ?
            ");
            $stmt->execute([$title, $content, $excerpt, $author, $category_id, $icon, $status, $replies, $views, $id]);

            set_flash('success', 'Forum post updated successfully!');
            header('Location: index.php');
            exit;
        } catch (Exception $e) {
            $errors[] = 'Error updating forum post: ' . $e->getMessage();
        }
    }
    ...
    $_POST['icon'] = $post['icon'];
    $_POST['status'] = $post['status'];
    $_POST['replies'] = $post['replies'];
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="btn-group">
        <a href="view.php?id=<?php echo $post['id']; ?>" class="btn btn-outline-info">
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="replies" name="replies"
                                   value="<?php echo e($_POST['replies']); ?>" min="0">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="views" name="views"
                                   value="<?php echo e($_POST['views']); ?>" min="0">
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-1"></i> Update Forum Post
                    </button>
